...
// =============================================
const STORAGE_KEYS={settings:'db_settings',priceList:'db_price_list',suppliers:'db_suppliers',orders:'db_orders'};
...
var settings={};var priceList=[];var suppliers=[];var savedOrders=[];var currentOrderId=null;var roomCounter=0;var fabricCounters={};var sewingCounters={};var extraCounters={};
...
function readStorage(key,fallback){try{var value=JSON.parse(localStorage.getItem(key)||'null');return value===null?fallback:value;}catch(e){return fallback;}}

document.addEventListener('DOMContentLoaded',function(){loadAllData();addRoom();});

function loadAllData(){
    var ss=readStorage(STORAGE_KEYS.settings,null);
    ...
    var sp=readStorage(STORAGE_KEYS.priceList,null);priceList=sp||DEFAULT_PRICE_LIST.slice();if(!sp)localStorage.setItem(STORAGE_KEYS.priceList,JSON.stringify(priceList));
    var su=readStorage(STORAGE_KEYS.suppliers,null);suppliers=su||[];if(!su)localStorage.setItem(STORAGE_KEYS.suppliers,JSON.stringify(suppliers));
    savedOrders=readStorage(STORAGE_KEYS.orders,[]);if(!Array.isArray(savedOrders))savedOrders=[];
}
...
function saveOrders2Storage(){localStorage.setItem(STORAGE_KEYS.orders,JSON.stringify(savedOrders));}
...
function recalcTesma(rid){
    ...
    document.querySelectorAll('#fabric-rows-'+rid+' .item-row').forEach(function(row){
        ...
        if(['portera','portera_wave','portera_bant','tul','tul_wave','tul_bant'].indexOf(type)>=0&&qty>0){
...
function getTesmaTotal(rid){
    ...
    document.querySelectorAll('#fabric-rows-'+rid+' .item-row').forEach(function(row){
        var type=row.querySelector('select').value||'';var qty=parseFloat(row.querySelector('[id^="fq-"]').value)||0;
        if(['portera','portera_wave','portera_bant','tul','tul_wave','tul_bant'].indexOf(type)>=0&&qty>0)t+=Math.round((qty+0.2)*p);
    });return t;
}
...
function addSewingRow(rid){
    ...
    row.innerHTML='<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;"><div class="field-group field-type"><label>Вид</label><select onchange="onSewType('+rid+','+idx+')" id="st-'+rid+'-'+idx+'"><option value="">-- Выбрать --</option><option value="portera">Портьера</option><option value="portera_wave">Портьера волна</option><option value="portera_bant">Портьера бантовка</option><option value="tul">Тюль</option><option value="tul_wave">Тюль волна</option><option value="tul_bant">Тюль бантовка</option><option value="rimskaya">Римская штора</option><option value="pokryvalo">Покрывало</option></select></div><div id="sf-'+rid+'-'+idx+'" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;flex:1;"></div><div class="field-group field-total"><label>Итого</label><div class="item-total" id="stt-'+rid+'-'+idx+'">0 ₽</div></div><button class="btn btn-delete" onclick="remSewing('+rid+','+idx+')">✕</button></div><div id="se-'+rid+'-'+idx+'"></div>';
    c.appendChild(row);
}
...
function onSewType(rid,idx){
    ...
    if(['portera','portera_wave','portera_bant','tul','tul_wave','tul_bant'].indexOf(type)>=0){
...
function calculateSewingTotal(type,v){
    var A=v.A||0,B=v.B||0,W=v.W||0,D=v.D||0,K=v.K||1,P=v.P||0;var base=0;
    if(type==='portera')base=((A*3)+(B*2))*D;
    else if(type==='portera_wave')base=((A*3)+(B*2))*D+(A*(v.wavePrice||150));
    else if(type==='portera_bant')base=((A*3)+(B*2))*D+(W*(v.bantPrice||1200));
    else if(type==='tul')base=((A*2)+(B*2))*D;
    else if(type==='tul_wave')base=((A*2)+(B*2))*D+(A*(v.wavePrice||150));
    else if(type==='tul_bant')base=((A*2)+(B*2)+(W*4))*D;
    return Math.round(base*(1+P/100)*K);
}
function calcSew(rid,idx){
    ...
    var total=0;
    if(['portera','portera_wave','portera_bant','tul','tul_wave','tul_bant'].indexOf(type)>=0){
        total=calculateSewingTotal(type,{A:gv('sa-'+rid+'-'+idx),B:gv('sb-'+rid+'-'+idx),W:gv('sw-'+rid+'-'+idx),D:gv('sd-'+rid+'-'+idx),K:gv('sk-'+rid+'-'+idx)||1,P:gv('sp-'+rid+'-'+idx),wavePrice:settings.price_wave_markup||150,bantPrice:settings.price_bantovka||1200});
    }
...
function recalcVyv(rid){
    ...
    document.querySelectorAll('#fabric-rows-'+rid+' .item-row').forEach(function(row){
        var type=row.querySelector('select').value||'';var qty=parseFloat(row.querySelector('[id^="fq-"]').value)||0;
        if(['portera','portera_wave','portera_bant','tul','tul_wave','tul_bant'].indexOf(type)>=0)meters+=qty;
    });
...
function getVyvTotal(rid){
    ...
    document.querySelectorAll('#fabric-rows-'+rid+' .item-row').forEach(function(row){
        var type=row.querySelector('select').value||'';var qty=parseFloat(row.querySelector('[id^="fq-"]').value)||0;
        if(['portera','portera_wave','portera_bant','tul','tul_wave','tul_bant'].indexOf(type)>=0)m+=qty;
...
function calcServices(rid){
    ...
    var cm=parseFloat((document.getElementById('scm-'+rid)||{}).value)||0;var cp=parseFloat((document.getElementById('scp-'+rid)||{}).value)||0;var ct=Math.round(cm*cp);setT('sct-'+rid,ct);
    ...
    setT('sgt-'+rid,it+ct+dl+ds);recalcRoom(rid);
}
function getServicesTotal(rid){
    ...
    var cm=parseFloat((document.getElementById('scm-'+rid)||{}).value)||0;var cp=parseFloat((document.getElementById('scp-'+rid)||{}).value)||0;
    var dl=parseFloat((document.getElementById('sdl-'+rid)||{}).value)||0;var ds=parseFloat((document.getElementById('sds-'+rid)||{}).value)||0;
    return Math.round(im*ip)+Math.round(cm*cp)+Math.round(dl)+Math.round(ds);
}
...
function recalcGrandTotal(){
    ...
    var discount=Math.max(0,parseFloat((document.getElementById('discountAmount')||{}).value)||0);
    var gt=document.getElementById('grandTotal');if(gt)gt.textContent=fmt(Math.max(0,grand-discount));
}
function recalcAll(){
...
function captureOrderData(){
    ...
    document.querySelectorAll('.room').forEach(function(room){
        rooms.push({
            roomId:parseInt(room.dataset.roomId),
            html:room.innerHTML,
            fields:Array.prototype.map.call(room.querySelectorAll('input,select,textarea'),function(field){return field.value;}),
            vyvRemoved:!!((room.querySelector('[id^="vyveshivanie-block-"]')||{}).dataset||{}).removed
        });
    });
    return {
        version:1,
        ...
        discount:Math.max(0,parseFloat(document.getElementById('discountAmount').value)||0),
        roomCounter:roomCounter,
        fabricCounters:Object.assign({},fabricCounters),
        sewingCounters:Object.assign({},sewingCounters),
        extraCounters:Object.assign({},extraCounters),
        rooms:rooms
    };
}

function saveOrder(){
    var existing=savedOrders.find(function(order){return order.id===currentOrderId;});
    var client=document.getElementById('clientName').value.trim();
    var suggestion=existing?existing.title:(client||'Заказ от '+new Date().toLocaleDateString('ru-RU'));
    document.getElementById('saveOrderName').value=suggestion;document.getElementById('saveOrderModal').classList.add('active');document.getElementById('saveOrderName').focus();
}
function closeSaveOrderModal(){document.getElementById('saveOrderModal').classList.remove('active');}
function confirmSaveOrder(){
    var existing=savedOrders.find(function(order){return order.id===currentOrderId;});
    var title=document.getElementById('saveOrderName').value.trim();if(!title)return;
    var id=existing?existing.id:String(Date.now());
    var order={id:id,title:title,updatedAt:new Date().toISOString(),total:(document.getElementById('grandTotal')||{}).textContent||'0 ₽',data:captureOrderData()};
    var index=savedOrders.findIndex(function(item){return item.id===id;});
    if(index>=0)savedOrders[index]=order;else savedOrders.unshift(order);
    currentOrderId=id;saveOrders2Storage();closeSaveOrderModal();
    var button=document.querySelector('.header-buttons button[onclick="saveOrder()"]');if(button){var old=button.textContent;button.textContent='✓ Заказ сохранён';setTimeout(function(){button.textContent=old;},1800);}
}

function openOrdersModal(){savedOrders=readStorage(STORAGE_KEYS.orders,[]);renderOrdersList();document.getElementById('ordersModal').classList.add('active');}
...
function renderOrdersList(){
    ...
    if(!savedOrders.length){c.innerHTML='<div class="orders-empty">Сохранённых заказов пока нет.</div>';return;}
    c.innerHTML='<table><thead><tr><th>Заказ</th><th>Сумма</th><th></th></tr></thead><tbody>'+savedOrders.map(function(order){
        var date=new Date(order.updatedAt);var dateText=isNaN(date.getTime())?'':date.toLocaleString('ru-RU');
        return '<tr><td><strong>'+escHTML(order.title)+'</strong><div class="order-meta">'+escHTML(dateText)+'</div></td><td>'+escHTML(order.total||'')+'</td><td><div class="order-actions"><button class="btn btn-add" onclick="loadOrder(\''+order.id+'\')">Открыть</button><button class="btn btn-delete" onclick="deleteSavedOrder(\''+order.id+'\')">✕</button></div></td></tr>';
    }).join('')+'</tbody></table>';
}
function loadOrder(id){
    var order=savedOrders.find(function(item){return item.id===id;});if(!order||!order.data)return;
    var data=order.data;var container=document.getElementById('roomsContainer');container.innerHTML='';
    roomCounter=parseInt(data.roomCounter)||0;fabricCounters=data.fabricCounters||{};sewingCounters=data.sewingCounters||{};extraCounters=data.extraCounters||{};
    (data.rooms||[]).forEach(function(savedRoom){
        var room=document.createElement('div');room.className='room';room.id='room-'+savedRoom.roomId;room.dataset.roomId=savedRoom.roomId;room.innerHTML=savedRoom.html;container.appendChild(room);
        var fields=room.querySelectorAll('input,select,textarea');(savedRoom.fields||[]).forEach(function(value,index){if(fields[index])fields[index].value=value;});
        var vyv=room.querySelector('[id^="vyveshivanie-block-"]');if(vyv&&savedRoom.vyvRemoved)vyv.dataset.removed='true';
    });
    document.getElementById('clientName').value=data.clientName||'';document.getElementById('clientPhone').value=data.clientPhone||'';document.getElementById('clientAddress').value=data.clientAddress||'';
    document.getElementById('discountAmount').value=data.discount||0;currentOrderId=id;closeOrdersModal();
    if(!document.querySelector('.room'))addRoom();
    refreshLoadedOrder();
}
function refreshLoadedOrder(){
    document.querySelectorAll('.room').forEach(function(room){
        var rid=parseInt(room.dataset.roomId);recalcTesma(rid);recalcVyv(rid);
        room.querySelectorAll('[id^="st-'+rid+'-"]').forEach(function(select){var idx=parseInt(select.id.split('-').pop());calcSew(rid,idx);});
        if(document.getElementById('ct-'+rid))calcCornice(rid);if(document.getElementById('fgt-'+rid))calcFur(rid);if(document.getElementById('sgt-'+rid))calcServices(rid);calcExtra(rid);recalcRoom(rid);
    });
    recalcGrandTotal();
}
function deleteSavedOrder(id){
    var order=savedOrders.find(function(item){return item.id===id;});if(!order||!confirm('Удалить сохранённый заказ «'+order.title+'»?'))return;
    savedOrders=savedOrders.filter(function(item){return item.id!==id;});if(currentOrderId===id)currentOrderId=null;saveOrders2Storage();renderOrdersList();
}

function newOrder(){
    if(!confirm('Создать новый заказ? Все данные будут очищены.'))return;
    currentOrderId=null;roomCounter=0;fabricCounters={};sewingCounters={};extraCounters={};
...
function buildPrintHTML(){
    ...
    var subtotal=0;document.querySelectorAll('.subtotal-value').forEach(function(el){subtotal+=prs(el.textContent);});
    var discount=Math.max(0,parseFloat((document.getElementById('discountAmount')||{}).value)||0);var grandTotal=Math.max(0,subtotal-discount);
    ...
    document.querySelectorAll('.room').forEach(function(room){
        ...
        if(extras.length){
            h+='<div style="margin-bottom:8px;"><strong>Непредвиденные:</strong><table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:4px;"><tr style="background:#f3f0ff;"><th style="text-align:left;padding:3px 6px;">Наименование</th><th style="text-align:left;padding:3px 6px;">Артикул</th><th style="text-align:right;padding:3px 6px;">Кол-во</th><th style="text-align:right;padding:3px 6px;">Цена</th><th style="text-align:right;padding:3px 6px;">Итого</th></tr>';
            extras.forEach(function(row){var nm=(row.querySelector('.extra-name')||{}).value||'—';var art=(row.querySelector('.extra-article')||{}).value||'—';var qty=(row.querySelector('[id^="eq-"]')||{}).value||'0';var price=(row.querySelector('[id^="ep-"]')||{}).value||'0';var total=(row.querySelector('[id^="ett-"]')||{}).textContent||'0 ₽';h+='<tr><td style="padding:3px 6px;">'+escHTML(nm)+'</td><td style="padding:3px 6px;">'+escHTML(art)+'</td><td style="text-align:right;padding:3px 6px;">'+qty+'</td><td style="text-align:right;padding:3px 6px;">'+price+'</td><td style="text-align:right;padding:3px 6px;">'+total+'</td></tr>';});
            h+='</table></div>';
        }
        h+='<div style="text-align:right;font-weight:bold;color:#6b4c9a;border-top:1px solid #eee;padding-top:6px;">ПОДИТОГ: '+sub+'</div></div>';
    });
    h+='<div style="margin-top:25px;text-align:right;border-top:3px solid #6b4c9a;padding-top:10px;">';
    if(discount>0)h+='<div style="font-size:14px;color:#555;margin-bottom:5px;">Сумма до скидки: '+fmt(subtotal)+'</div><div style="font-size:14px;color:#b91c1c;margin-bottom:5px;">Скидка: −'+fmt(discount)+'</div>';
    h+='<div style="font-size:20px;font-weight:bold;color:#6b4c9a;">ИТОГО: '+fmt(grandTotal)+'</div></div>';
    return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>DECOR BONJOUR</title><style>body{font-family:Arial,sans-serif;font-size:13px;color:#333;padding:30px;max-width:800px;margin:0 auto;}table{border:1px solid #ddd;}th,td{border-bottom:1px solid #eee;}@media print{body{padding:10px;}}</style></head><body>'+h+'</body></html>';
}
